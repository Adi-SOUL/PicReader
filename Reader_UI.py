...
def toplevel_with_bar(w: int, h: int, x: int, y: int, name: str, label_: str, large: bool, master=None) -> tkinter.Toplevel:
	if master:
...
class CollapsingFrame(ttk.Frame):
	...
	def __init__(self, master, size, **kwargs):
		...
		self.columnconfigure(1, weight=1, minsize=int(size/3))
		self.cumulative_column = 0
		self.child = None

		self.images = [ttk.PhotoImage(file='Pics/up.png'), ttk.PhotoImage(file='Pics/right.png')]
...
class ReaderUI:
	def __init__(self):
		...
		self.font_size = int(20 * self.scaling_ratio) if int(20 * self.scaling_ratio) < 20 else 20
		...
		self.side_frame = ttk.Frame(self.side_c_frame)
		...
		self.file_menu = ttk.Menu(self.menu_bar, font=('Consolas', 15), borderwidth=0)
		self.tool_menu = ttk.Menu(self.menu_bar, font=('Consolas', 15), borderwidth=0)
		self.convert_menu = ttk.Menu(self.file_menu, font=('Consolas', 15), borderwidth=0)
		self.themes_menu = ttk.Menu(self.file_menu, font=('Consolas', 15), borderwidth=0)
	...
	def change_grid(self) -> None:
		...
		self.main_frame.grid(row=0, column=1, sticky=tkinter.NSEW)
